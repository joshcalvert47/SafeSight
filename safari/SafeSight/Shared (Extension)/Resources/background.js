...
// Safari supports native "background service workers" (Safari 16.4+) but they
// cannot use chrome.offscreen, and older Safari versions only support event
// pages. Inference therefore runs directly in this persistent background
// context, exactly like the Firefox build.
//
// Safari also has no chrome.contextMenus, so the per-site allow/block and
// per-tab toggle features from the Chrome build are not available here. The
// popup still controls every global setting.
let readyModel = null;
...
async function analyzePayload(payload) {
    ...
    // payload.data arrives as a plain array (JSON messaging), matching the
    // Firefox build's wire format.
...
// ---------------------------------------------------------------------------
// CORS-free image proxy (same as the Chrome/Firefox builds)
// Content scripts cannot read pixels of cross-origin images without CORS
// headers. The background page has <all_urls> host permissions, so it can
// fetch and hand back the raw bytes. Messaging is JSON-only: plain Array.
// ---------------------------------------------------------------------------
