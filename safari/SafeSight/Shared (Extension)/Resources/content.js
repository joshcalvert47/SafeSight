// content.js (Safari)
// Same pipeline as the Chrome build. Safari-specific adaptations:
...
//   - OffscreenCanvas is not exposed to content scripts on iOS, so the shared
//     224x224 draw target is a detached DOM canvas.
...
let uniqueId = 0;
let siteFilteringEnabled = true; // Per-site master toggle (popup / storage driven).

// Safari MV3 lacks exclude_matches — keep these in sync with the Chrome build.
...
if (!isExcludedPage()) {
    ...
    chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
        if (message.type === 'AI_RESULT') {
            ...
            // Update Stats (throttled; the background page owns the persisted values)
        ...
        if (message.type === 'SITE_TOGGLE') {
            // Live switch of filtering for this tab.
    ...
    // ask the background page to fetch it (host_permissions bypass CORS there).
    async function extractPixels(img) {
        ...
        try {
            ...
            // Tainted canvas (no CORS headers) — fall through to background fetch.
    ...
    function drawBytesToCanvas(bytes, mime) {
        ...
        return new Promise((resolve, reject) => {
            ...
            tImg.onerror = () => { URL.revokeObjectURL(url); reject(new Error("Fetched image failed to decode")); };
            tImg.src = url;
        });
    }
    ...
    async function runAnalysis(img) {
        ...
        try {
            ...
            chrome.runtime.sendMessage({
                type: 'ANALYZE',
                id,
                // Extension messaging is JSON-only: typed arrays arrive as {}
